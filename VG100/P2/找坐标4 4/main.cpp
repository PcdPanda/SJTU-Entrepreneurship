...
volatile int L[17];//模块左，前，右通断情况 ，数据为通向的模块号 
volatile int F[17];//-1代表未知，0代表墙 
volatile int R[17];
volatile int l[17];//模块左，前，右优先级，优先走大的 
volatile int f[17];
volatile int r[17];
volatile int D[17];//机器人到模块时的方向，返回时用 
volatile int i=1;//当前探索总模块数 
volatile int E=1;//当前机器人模块位置 
volatile int X[17]={0},Y[17]={0};//机器人当前位置及模块坐标 
